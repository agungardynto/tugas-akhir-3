...
<script>
    ...
    $('.tags').select().val({!!
        json_encode($data->tag()->allRelatedIds())
    !!}).triger('change');
</script>
@endpush
@section('content')
<div class="row">
    <div class="col-12">
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('blog.update', $data->id) }}" id="basicform" method="post" enctype="multipart/form-data">
                    @method('put')
                    ...
                    <div class="form-group">
                        ...
                        <input id="title" type="text" name="title" placeholder="Enter Blog Title" value="{{ $data->title }}" autocomplete="off" class="form-control @error('title') is-invalid @enderror">
                    </div>
                    <div class="form-group">
                        <label for="post">Blog Posts</label>
                        <textarea name="post" id="post">{{ $data->post }}</textarea>
                        @error('post')
                            {{ $message }}
                        @enderror
                    </div>
